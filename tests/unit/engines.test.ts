...
describe('SmartERP Core Engines & Localization Suite', () => {
  const mockTenant: Tenant = {
    ...
    enabled_modules: ['SALES', 'PRODUCTS', 'INVENTORY', 'CUSTOMERS', 'ACCOUNTING', 'REPORTS', 'IMEI', 'REPAIRS', 'RECHARGE'],
    created_at: new Date().toISOString(),
  ...
  describe('1. RBAC Engine (Role-Based Access Control)', () => {
    it('SUPER_ADMIN should have unrestricted full access to any permission', () => {
      expect(RbacEngine.hasPermission('SUPER_ADMIN', 'dashboard.view')).toBe(true);
      expect(RbacEngine.hasPermission('SUPER_ADMIN', 'system.super_admin_matrix')).toBe(true);
      expect(RbacEngine.hasPermission('SUPER_ADMIN', 'any.custom.perm')).toBe(true);
    });

    it('CASHIER should have POS sales access but not admin settings or void privileges', () => {
      expect(RbacEngine.hasPermission('CASHIER', 'sales.pos_access')).toBe(true);
      expect(RbacEngine.hasPermission('CASHIER', 'system.settings_manage')).toBe(false);
      expect(RbacEngine.hasPermission('CASHIER', 'sales.void_invoice')).toBe(false);
    });

    it('ADMIN should have full operational access to store administration', () => {
      expect(RbacEngine.hasPermission('ADMIN', 'system.staff_manage')).toBe(true);
      expect(RbacEngine.hasPermission('ADMIN', 'accounting.view_ledger')).toBe(true);
  ...
  describe('2. Navigation Engine', () => {
    it('generates proper navigation items for ADMIN role based on enabled modules', () => {
      const navItems = NavigationEngine.getDynamicNavigation(mockTenant, 'ADMIN');
    ...
    it('includes Super Admin exclusive modules only for SUPER_ADMIN role', () => {
      const superNav = NavigationEngine.getDynamicNavigation(mockTenant, 'SUPER_ADMIN');
  ...
  describe('3. Rule Engine', () => {
    ...
    it('validates cart item tracking requirements properly', () => {
      const genericValidation = RuleEngine.validateCartItemRequirements(sampleProduct, { quantity: 2 });
      ...
      const invalidImeiValidation = RuleEngine.validateCartItemRequirements(imeiProduct, { quantity: 1 });
  ...
  describe('4. Workflow Engine & Accounting Pipeline', () => {
    it('processes a complete POS sale transaction and posts double-entry journal vouchers', () => {
      ...
      expect(saleEntry?.debit_account).toBe('ক্যাশ ড্রয়ার / কাউন্টার ক্যাশ (Cash in Hand)');
      expect(saleEntry?.credit_account).toBe('পণ্য বিক্রয় রাজস্ব (Sales Revenue)');
    });
  });
  ...
  describe('5. i18n Localization & Translation Engine', () => {
    ...
    it('formats Bengali numbers and currency correctly', () => {
      ...
      expect(i18n.formatCurrency(524.50)).toBe('৳ 524.50');
    });
  });
});
